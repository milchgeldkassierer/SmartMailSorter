...
describe('formatTimeAgo', () => {
  ...
  describe('Weeks Range (7+ days)', () => {
    it('should format exactly 7 days as "vor mehr als einer Woche"', () => {
      const now = Date.now();
      const timestamp = now - 7 * 24 * 60 * 60 * 1000; // 7 days ago
      const result = formatTimeAgo(timestamp);
      expect(result).toBe('vor mehr als einer Woche');
    });

    it('should format 14 days as "vor mehr als einer Woche"', () => {
      const now = Date.now();
    ...
    it('should format 30 days as "vor mehr als einer Woche"', () => {
      const now = Date.now();
    ...
    it('should format 365 days as "vor mehr als einer Woche"', () => {
      const now = Date.now();
  ...
  describe('Boundary Testing', () => {
    it('should correctly transition from seconds to minutes', () => {
      ...
      // 59 seconds should be "vor wenigen Sekunden"
      const justUnder = now - 59 * 1000;
      expect(formatTimeAgo(justUnder)).toBe('vor wenigen Sekunden');

      // 60 seconds (1 minute) should be "vor 1 Minute"
      const justOver = now - 60 * 1000;
      expect(formatTimeAgo(justOver)).toBe('vor 1 Minute');
    });
    ...
    it('should correctly transition from minutes to hours', () => {
      ...
      // 60 minutes (1 hour) should be "vor 1 Stunde"
      const justOver = now - 60 * 60 * 1000;
      expect(formatTimeAgo(justOver)).toBe('vor 1 Stunde');
    });
    ...
    it('should correctly transition from hours to days', () => {
      ...
      // 24 hours (1 day) should be "vor 1 Tag"
      const justOver = now - 24 * 60 * 60 * 1000;
      expect(formatTimeAgo(justOver)).toBe('vor 1 Tag');
    });
    ...
    it('should correctly transition from days to weeks', () => {
      ...
      // 7 days should be "vor mehr als einer Woche"
      const justOver = now - 7 * 24 * 60 * 60 * 1000;
      expect(formatTimeAgo(justOver)).toBe('vor mehr als einer Woche');
    });
  });
  ...
  describe('German Language Correctness', () => {
    it('should always use German words', () => {
      const testCases = [
        { time: 30 * 1000, expected: 'vor' }, // seconds
    ...
    it('should use correct singular/plural forms', () => {
      ...
      // Singular forms
      expect(formatTimeAgo(now - 1 * 60 * 1000)).toBe('vor 1 Minute');
      expect(formatTimeAgo(now - 1 * 60 * 60 * 1000)).toBe('vor 1 Stunde');
      expect(formatTimeAgo(now - 1 * 24 * 60 * 60 * 1000)).toBe('vor 1 Tag');
  ...
  describe('Type Safety', () => {
    ...
    it('should always return string starting with "vor" for valid timestamps', () => {
      const testCases = [
        Date.now(),
        Date.now() - 30 * 1000,
        Date.now() - 5 * 60 * 1000,
