...
const EmailView: React.FC<EmailViewProps> = ({ email, searchQuery, categories = [], onCategoryChange }) => {
    ...
    <div className="flex-1 bg-slate-50 flex flex-col h-full overflow-hidden">
        ...
        <div className="flex items-center justify-between">
          ...
          {email.smartCategory && (
            <div className="relative" ref={categoryDropdownRef}>
              ...
              {/* Dropdown Menu */}
              {isCategoryDropdownOpen && onCategoryChange && categories.length > 0 && (
                <div className="absolute right-0 mt-2 w-56 bg-white border border-slate-200 rounded-lg shadow-lg z-50 max-h-80 overflow-y-auto">
                  {categories.map((cat) => (
                    ...
                    >
                      ...
                      <span className="text-sm font-medium">{getCategoryDisplayName(cat.name)}</span>
                    </button>
                  ))}
                </div>
              )}
      ...
      {showHtml && hasHtml && !imagesLoaded && blockedImageCount > 0 && (
        <div className="mx-6 mt-2 flex items-center justify-between bg-amber-50 border border-amber-200 rounded-lg px-4 py-2 flex-shrink-0">
          <span className="text-sm text-amber-800">{t('emailView.blockedImages', { count: blockedImageCount })}</span>
          <button
            type="button"
