        ...
        {emails.slice(0, visibleCount).map((email) => {
          ...
          return (
            <div
              ...
              className={`group relative p-4 pl-12 border-b border-slate-100 cursor-pointer hover:bg-slate-50 transition-colors ${selectedEmailId === email.id ? 'bg-blue-50 border-l-4 border-l-blue-600' : 'border-l-4 border-l-transparent'
            ...
            >
              ...
              <div className="absolute top-2 right-2 flex items-center gap-1">
                ...
                <button
                  onClick={(e) => { e.stopPropagation(); onToggleFlag(email.id); }}
                  className={`p-1.5 rounded-full transition-all ${email.isFlagged
              ...
              <div className="flex justify-between items-start mb-1 pr-6">
                <span className={`font-medium truncate max-w-[65%] ${email.isRead ? 'text-slate-600' : 'text-slate-900'}`}>
                  {email.sender}
                </span>
              ...
              <div className={`text-sm mb-1 truncate pr-8 flex items-center gap-2 ${email.isRead ? 'text-slate-500' : 'text-slate-800 font-medium'}`}>
